...
3. Run the reusable script for the selected exact names. Keep its sequential request pacing enabled to reduce Steam `429` responses.
4. Present one item per row, preserve the user's item order, and link each name to its Steam market page.
5. State the snapshot time, target currency, source, and the historical-price limitation below the table.
